...
- 新工程尚未在根目录初始化；这是刻意保留的阶段边界。
- 当前完成的是“第 0 轮：盘点、归档、路线设计”，不是功能迁移完成。

> [!WARNING]
> `legacy/` 被忽略后，新克隆仓库不会包含它。长期可追溯的旧代码来源是当前分支迁移前的 Git 提交/标签，而不是本机 `legacy/` 目录。开始第 1 轮前，建议给迁移前提交打一个标签，例如 `legacy-vue-cli-baseline`。
...
| 1 | 初始化现代工程骨架 | 待开始 | Bun 锁文件、Vue/Vite/TS 配置、空壳应用 | 类型检查、单测、Lint、构建全部通过 |
| 2 | 迁移平台基础设施 | 待开始 | 环境变量、样式、SVG、Mock、HTTP 客户端 | 基础设施测试通过，不迁业务页 |
...
```text
请执行 docs/01-migration-roadmap.md 的第 1 轮，只初始化 Bun + Vue + TypeScript 现代工程骨架。
不要迁移 legacy 业务代码；保留 /legacy/ 忽略规则和 docs/。
完成后运行 type-check、unit test、lint、build，并更新 docs 中的轮次状态和变更记录。
```

第 1 轮验收完成后再开始第 2 轮，避免“脚手架问题”和“业务迁移问题”混在一次变更中。
