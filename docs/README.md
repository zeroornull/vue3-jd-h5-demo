...
- 当前完成到“第 1 轮：现代工程骨架”；旧业务、Vant、Axios、Mock 和历史页面尚未迁移。
...
```text
请执行 docs/01-migration-roadmap.md 的第 2 轮，只迁移平台基础设施：
Vite 环境变量、类型化 Axios 客户端、开发 Mock、移动端样式和 SVG。
不要迁移业务页面、旧 Router 模块或 Vuex；保留 /legacy/ 忽略规则。
先补基础设施测试，完成后运行 frozen install、type-check、unit、lint、build，并更新文档。
```

第 2 轮验收完成后再开始 Router/Pinia 迁移，避免基础设施问题与业务状态迁移混在一次变更中。
