...
- 当前完成到“第 4 轮：公共组件与首个纵向切片”；Home → Search → Cart 已具备类型化 API、Pinia、Vant 4 交互和移动端页面。
- 56 条旧模块路由中 3 条已迁移、53 条仍显示迁移占位页；订单、钱包、节点、个人中心等域尚未开始。
...
```text
请执行 docs/01-migration-roadmap.md 的第 5 轮第一个业务子域：
商品 / 分类 / 榜单 / 秒杀。
只迁移 /classify、/classify/product、/classify/recommend、秒杀、榜单和它们直接依赖的 API、组件、资源与测试；
不要同时迁移订单、钱包、节点或个人中心。完成后更新路由矩阵状态并运行全部质量门和 375/390/430px 浏览器验证。
```

商品域完成后，再按认证、订单、钱包、节点、个人中心等边界逐域迁移，避免一次性复制剩余旧 SFC。
