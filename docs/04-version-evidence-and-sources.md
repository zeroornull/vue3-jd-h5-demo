...
第 5 轮订单子域日期：2026-08-30。没有新增外部依赖。已提交为 `84dcb6a`。
...
第 5 轮个人中心日期：2026-08-30。没有新增外部依赖。已提交为 `5fb24f6`。

数据与状态证据：`useProfileStore` 覆盖加载、改昵称、保存地址、反馈和错误恢复；Mock 覆盖资料、改密、新增地址和反馈。Auth Store 增加 `updateUser` 持久化昵称。

路由证据：新增 13 条 migrated，矩阵为 42 migrated / 14 pending-view。`/mine` 与 `/setting/aboutAs` 受登录守卫保护。

完整验证：22 test files / 65 tests；502 modules transformed。主入口 127.46 kB（gzip 44.99 kB）。浏览器：个人中心、改昵称、地址默认/新增、关于我们、退出登录守卫均通过；375/430px 无横向溢出。

## 14. 本仓库第 5 轮钱包子域落地证据

第 5 轮钱包子域日期：2026-08-30。没有新增外部依赖。已提交为 `9e6f6cd`。

数据与状态证据：`useWalletStore` 覆盖加载、流水筛选和矿池领取；Mock 覆盖 snapshot 与 claim。

路由证据：新增 6 条 migrated，矩阵为 48 migrated / 8 pending-view。余额/消费明细共享页面，三个矿池共享页面。`/wallet` 与 `/pool` 受登录守卫保护。

完整验证：23 test files / 69 tests；512 modules transformed。主入口 128.19 kB（gzip 45.13 kB）。浏览器：钱包总览、矿池领取、余额流水筛选均通过；375/430px 无横向溢出。

## 15. 本仓库第 5 轮节点申请落地证据

第 5 轮节点申请日期：2026-08-30。没有新增外部依赖。代码仍在工作区，未形成提交。

数据与状态证据：`useNodeStore` 覆盖加载与申请扣减份数；Mock 覆盖节点目录和分享节点支付。

路由证据：新增 6 条 migrated，矩阵为 54 migrated / 2 pending-view。5 类申请页共享 `NodeApplyView`。`/node` 受登录守卫保护。

完整验证：24 test files / 72 tests；520 modules transformed。主入口 128.93 kB（gzip 45.24 kB）。浏览器：分享节点 CoinPay 剩余 128→127；区域节点支付后剩余 65→64；375/430px 无横向溢出。
...
路由证据：新增 2 条 migrated，矩阵为 56 migrated / 0 pending-view。`/myFocus` 受登录守卫保护；`/storeDetail` 保持公开。
...
- 第 5 轮认证补全了旧登录页空处理函数留下的闭环，并新增客户端守卫；这不是旧 Router 行为的机械翻译。改密、个人资料当时未作为该子域完成证据；HTTP 401/403 跳转已在路线之后的独立变更中补上；
- 第 5 轮订单补全了旧静态订单页留下的闭环，并把购物车结算接到创建订单。真实支付、库存扣减、商家会话和店铺详情仍未迁移；
- 第 5 轮个人中心补全了资料/地址/设置闭环。关注和真实短信通道仍未迁移；国家/地区与省市区使用精简列表，没有搬旧 `area.js`；
